...
flowchart TD
    ...
    subgraph Drop14["【計画】Drop 14: クロール成否判定の厳格化とデータ品質ガード"]
        N1["CR-12: 資料件数・日付・タイトルの整合性による成否（success/partial/failed）自動判定精緻化"]
        N2["CR-13: プレースホルダー日付（2099/01/01）の自動検知・アラート機構"]
    end

    subgraph Drop15["【計画】Drop 15: 議事録・資料テキストの自動抽出・全文検索インデックス"]
        O1["CR-14: PDF資料・議事録HTML本文のメタデータ抽出パイプライン"]
        O2["CR-15: 会議体横断の議題・発言トピック検索基盤の整備"]
    end
...
## 4. アーカイブ：実装済み一覧（全57件・Drop 1 〜 12 完了）

<details>
<summary>Drop 1 〜 12 で完了した全57件のリファクタリング一覧（クリックで開閉）</summary>
