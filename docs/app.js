...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    return MEETINGS.filter(meeting => {
      ...
      if (state.searchQuery) {
        ...
        const match = queries.every(q => {
          const mTitle = meeting.title || meeting.name || '';
          const titleMatch = mTitle.toLowerCase().includes(q);
          ...
          const matMatch = meeting.materials ? meeting.materials.some(m => (m.name || m.title || '').toLowerCase().includes(q)) : false;
    ...
    return `
      <article class="timeline-card card-glass" id="meeting-${meeting.id}" style="--card-accent-color: ${minInfo.color}">
        ...
        <div class="card-title-block">
          ...
          <h3 class="card-title">
            ${escapeHtml(meeting.title || meeting.name || '')}
      ...
      if (state.searchQuery) {
        ...
        const match = queries.every(q => {
          ...
          const matchMeetings = councilMeetings.some(m => {
            ...
            return mTitle.toLowerCase().includes(q) ||
              (m.summary ? m.summary.toLowerCase().includes(q) : false) ||
              (m.tags ? m.tags.some(t => t && t.toLowerCase().includes(q)) : false) ||
              (m.materials ? m.materials.some(mat => (mat.name || mat.title || '').toLowerCase().includes(q)) : false);
          });
  ...
  function openModal(meeting) {
    ...
    el.modalTitle.textContent = meeting.title || meeting.name || '';
  ...
  function copyCitationText() {
    ...
    const citation = `${minName}「${m.title || m.name || ''}」（${formatDate(m.date)}開催）政策会議ウォッチ 参照: ${m.officialUrl}`;
  ...
  function exportFilteredData() {
    ...
    const csvRows = list.map(m => {
      ...
      return [
        ...
        `"${sanitizeCsvField((m.title || m.name || '').replace(/"/g, '""'))}"`,
